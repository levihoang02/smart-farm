...
class TimeSeriesCompressor:
    ...
    def perform_sparse_coding(self, segments):
        model = None
        if(self.sparseAlgo == "omp"):
          model =  OrthogonalMatchingPursuit(n_nonzero_coefs=self.n_nonzero_coefs)
        if(self.sparseAlgo == "lasso"):
          model = Lasso(alpha=0.1)
        if(self.sparseAlgo == "ridge"):
          model = Ridge(alpha=1.0)
        sparse_codes = []
        for segment in segments:
            sparse_code = model.fit(self.dictionary.T, segment).coef_
    ...
    def reverse_preprocessing(self, decompressed_data):
        # Reverse Standardization
        # Use the same scaler instance used during preprocessing
        # Since we've already standardized the data with scaler.fit_transform
        # we can use the scaler.inverse_transform to revert the data back to its original scale
        original_data = self.scaler.inverse_transform(decompressed_data.T).T

        # Reverse Detrending
        # To reverse detrending, we need to add the linear trend back.
        # Assuming a simple linear trend model, we can estimate the trend from the data:
        for i in range(original_data.shape[0]):
            # Recreate the linear trend based on the detrended data
            trend = np.polyfit(np.arange(original_data.shape[1]), original_data[i], 1)
            linear_trend = np.polyval(trend, np.arange(original_data.shape[1]))
            original_data[i] += linear_trend
    ...
    def evaluate_compression(self, original_data, decompressed_segments):
        ...
        print("Original")
        print(self.reverse_preprocessing(original_data))
        print("Decompressed")
        print(self.reverse_preprocessing(decompressed_data))
